...
module ParallelAsync =
    ...
    type ParallelAsync =
        ...
        static member zipWithStartChild
            (a1: Async<'left>)
            (a2: Async<'right>)
            : Async<'left * 'right> =
                ...
                fun c1 ->
                    async.Bind(
                        ...
                        fun c2 ->
                            async.Bind(
                                c1,
                                (fun r1 -> async.Bind(c2, (fun r2 -> async.Return(r1, r2))))
                            )
                    )
            )
            ...
            async.Bind(
                ...
                fun ct ->
                    ...
                    async.Bind(
                        Async.AwaitTask t1,
                        fun t1r ->
                            async.Bind(Async.AwaitTask t2, (fun t2r -> async.Return(t1r, t2r)))
                    )
            )
    ...
    type ParallelAsyncBuilderBase() =
        ...
        member inline _.BindReturn(x: Async<'T>, f) =
...
[<assembly: AutoOpen("IcedTasks.ParallelAsync.ParallelAsyncs")>]
do ()
